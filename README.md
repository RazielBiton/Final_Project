...
2. **Supabase (Auth & Edge Functions):** אחראי על Authentication וניהול מזהי משתמשים (כולל הפקת OTP לאיפוס סיסמה). כמו כן משמש להרצת פונקציות צד שרת Serverless כמו מערך Contact Us.
3. **Resend ותיבת תמיכה ייעודית:** שירות שליחת אימיילים מודרני המובנה בתוך פונקציית הקצה של Supabase לניתוב הודעות מטופס "צור קשר" ושליחתן ישירות לתיבת המייל הייעודית של צוות התמיכה: `easycare.support@gmail.com`.
...
* **`POST /api/auth/send-otp`** - הפקת קוד אימות ושליחתו למייל המשתמש באמצעות **Supabase Auth** לשם כניסה למערכת או לשם איפוס סיסמה בצורה מאובטחת.
